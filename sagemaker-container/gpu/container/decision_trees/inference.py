...
# arguments:
# - xTrain, yTrain, dydxTrain - data from simulation
# 
def doPredict(xTest, size, isDiff : bool, xTrain, yTrain, dydxTrain, weightSeed=None, deltidx=0):
    regressor = Neural_Approximator(xTrain, yTrain, dydxTrain)
    regressor.prepare(size, isDiff, weight_seed=weightSeed)
...
# the input is a dict of parameters.
def predict(input):
    xTest = list2arrt(input['xTest'])
    size = input['size']
    isDiff = input['isDiff'] 
    xTrain = list2arrt(input['xTrain'])
    yTrain = list2arrt(input['yTrain'])
    dydxTrain = list2arrt(input['dydxTrain'])
    return doPredict(xTest, size, isDiff, xTrain, yTrain, dydxTrain)
